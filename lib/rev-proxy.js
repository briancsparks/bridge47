...
lib.revProxy = function(req, res, destUrlObj /*, options, callback*/ ) {
  ...
  return sg.__run2({}, [function(result, next, last, abort) {
    ...
    if (cache[destHref]) {
      result.response = cache[destHref].response;
    ...
    return sg.curl(args, function(err, response) {
      ...
      // Cache the result
      if (shouldCache(destHref)) {
        cache[destHref] = sg.deepCopy({response, expires:_.now() + ttl});
        sg.setTimeout(10, function() {
          var expired = sg.reduce(cache, [], function(m, v, k) {
            if (_.now() > v.expires) { return sg.ap(m, k); }
            return m;
          });

          var i;
          for (i=0; i < expired.length; ++i) {
            delete cache[expired[i]];
          }
        });
      }
...
const noCache     = sg.keyMirror('feed,querySessions');
const toBeCached  = sg.keyMirror('getS3');
function shouldCache(url_) {
  const url = urlLib.parse(url_);

  if (_.last(_.compact(url.pathname.split('/'))) in toBeCached)    { return true; }
